...
import uk.ac.ncl.openlab.intake24.redux.foodsearch.{FoodSearchFailed, FoodSearchStarted, FoodSearchSuccessful}

import scala.scalajs.js
import scala.scalajs.js.annotation.{JSExport, JSExportTopLevel}

case class FoodNutrientsCalculatorState(selectedFood: Option[UserFoodHeader] = None)
...
class FoodSearch(val reduxStore: Store, val clientStore: Client, val selector: js.Array[String])
  extends ModuleStore[FoodNutrientsCalculatorState, FoodNutrientsCalculatorAction] with ApiUtils {

  private val foodDataService = new FoodDataImpl(clientStore.authRequestHandler)

  @JSExport
  val foodSearch = new FoodSearch(reduxStore, clientStore, selector.concat(Array("currentPrompt")))
